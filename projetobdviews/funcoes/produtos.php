<?php 
    ...
    function buscarProdutoPorId(int $id) : ?array{
        global $pdo;
        $stmt = $pdo->query("SELECT p.*, c.nome AS nome_categoria FROM produto p INNER JOIN categoria c ON c.id = p.categoria_id WHERE p.id = ?");
        $stmt->execute([$id]);
    ...
    function criarProduto(string $nome, string $descricao, float $preco, int $estoque_minimo, int $categoria_id): bool {
        global $pdo;
        $stmt = $pdo->prepare("INSERT INTO produto (nome, descricao, preco, estoque_minimo, categoria_id VALUES ( ?, ?, ?, ?, ?)");
